...
saveFile = matfile(strcat('Saved_predictions_NO_LQR_NA',date,'.mat'), 'Writable', true); %Note: writable is true by default IF the file does not exist
saveToIndex = 1;
...
while(1)
    ...
    if(any(oldDataBaseHolding ~= DataBaseHolding))
        ...
        output = SMPC_full_DW_NA(X0, time); 
