...
function setup(block)
  ...
  block.OutputPort(1).Dimensions = [24 1] ;
...
for i=1:reg_num
    block.Dwork(1).Data(i) = double(response(i)/100); 
...
function message = prepareReadingMessage(registry)
    ...
    transID=uint16(0);
    FunCod =  int16(4); % Function code to read "holding register" (3) / "input register" (4)
    ...
    reg_number = int16(19);%number of register to read
...
function response = readFloating(ModBusTCP, message)
    ...
    for i=1:reg_num
        response(i) = fread(ModBusTCP,1, 'int16');
    end
